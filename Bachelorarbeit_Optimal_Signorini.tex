...
\begin{definition}One dimensional Sobolev-Spaces \newline
	Let $p\in [1,\infty]$. For $k,p \in \mathbb{N}$, the Sobolev-Space $W^{k,p}(a,b)$ is defined as follows:
	\begin{align}
	\begin{split}
	...
	\end{equation}
	These results can be found in the literature, for example in \cite{mitrovic1997fundamentals}.
\end{definition}
...
\begin{lemma} Sobolev Embedding for $H^k(a,b)$ \newline
	For every $u \in H^k(a,b)$ there exists a $v \in C^{k-1}([a,b])$, such that $u = [v \vert_{(a,b)}]$. As an embedding, this can be written as:
	\begin{equation}
	H^k(a,b) \subset C^{k-1}([a,b])
	\end{equation}
	\begin{proof}
	We will start by showing that $H^1(a,b) \subset C^0([a,b])$. Let $u \in H^1(a,b)$, $a \leq x_1 < x_2 \leq b$, and let $u = [v]$ such that $v(x) = v_0 + \int_a^b w(\tau) \dif \tau$. We will show that $v$ is Hölder-continuous, which implies continuity:
	\begin{align*}
	...
	\end{align*}
	This case serves as an induction basis. For the induction step it suffices to apply the Fundamental Theorem of Calculus and integrate the continuous representative of the weak derivative to show that it is indeed the classical derivative.
	
	\textcolor{red}{QUESTION: Is this proof correct? I am pretty sure about the ideas, but maybe some detail is not completely precise. Should I elaborate on the induction step?}
	\end{proof}
\end{lemma}

Using this Lemma, we will henceforth identify elements of $H^k$ with their continuous representatives, allowing for the following statements involving pointwise conditions on elements of this space to be well-defined.

\begin{theorem} Equivalent formulation as a variational inequality\newline
