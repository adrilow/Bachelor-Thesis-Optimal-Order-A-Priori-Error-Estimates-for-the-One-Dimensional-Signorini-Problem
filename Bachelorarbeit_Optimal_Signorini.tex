...
The goal of this chapter is to analyze how good the solution to the discrete problem \eqref{eq:discrete_minimization} approximates the solution of the continuous one \eqref{eq:minimization_problem}. We will analyze the convergence order as a function of the mesh width $h$ with respect to several norms, and show that it is the optimal a-priori result \textcolor{red}{QUESTION: Reference?}.
...
\begin{lemma} Ritz Projection of a Signorini Solution \newline
	Suppose that $u$ solves \eqref{eq:variational_inequality} for some $f \in L^2(\Omega)$. Then, $R_h(u)$ is the unique solution $\tilde{u}_h$ of the following variational inequality:
...
\begin{lemma} Supercloseness Lemma \newline
	Let $f\in L^2(\Omega)$, $u$ the solution of \eqref{eq:variational_inequality} and $u_h$ the solution of \eqref{eq:discrete_variational_inequality}. Then the $H^1$-error between $u_h$ and the Ritz Projection of $u$ is bounded as follows:
...
\begin{definition} Lagrange-Interpolation \newline
	For a mesh width $h$ and corresponding nodes $1=t_0<t_1<\dots<t_N = 1$, where $h = t_{i+1} - t_i$, the unique interpolating polynomial of degree $N$ of a function $u$ and the nodes is defined as the Lagrange Interpolate $I_h(u)$.
	
	The well-definedness and uniqueness can be found in any standard numerics literature, such as \cite[Chapter 7]{NumerischeMathematik1}.
	
	\textcolor{red}{QUESTION: Is this the correct Lagrange-Interpolate? Because further down, when using (4.4) and Theorem 4.4.20 from Brenner, we use that $I_h(u) - R_h(u) \in V_h$}
\end{definition}

\begin{lemma}
	Let $u$ be the solution of \eqref{eq:variational_inequality}. Then there exists a $C \in \mathbb{R}_{>0}$ such that the following holds:
...
\begin{theorem} $L^\infty$ Error Estimate \newline
	Let $u$ be the solution of \eqref{eq:variational_inequality} and $u_h$ the solution of \eqref{eq:discrete_variational_inequality}. Then there exists a $C \in \mathbb{R}_{>0}$ such that the following holds:
	\begin{equation}
		\norm{u-u_h}_{L^\infty} \leq C h^{3/2}
	\end{equation}
\begin{proof}
	...
	\begin{align*}
	\norm{u-u_h}_{L^\infty} &\leq \norm{u-R_h(u)}_{L^\infty} + \norm{R_h(u) - u_h}_{L^\infty} \\
	\overset{\begin{subarray}{c} \text{Norm}	\\	\text{equivalence} \end{subarray}}&{\leq} C_1 \norm{u-R_h(u)}_{H^1(\Omega)} + \norm{R_h(u) - u_h}_{L^\infty} \\
	\overset{\eqref{eq:supercloseness} \eqref{eq:linfty_error_lemma}}&{\leq} 
	C_2 h^{3/2}\norm{u}_{H^2} + C_3 \max \left(\abs{(u-R_h(u))(-1)}, \abs{(u-R_h(u))(1)}\right) \\
	\overset{\text{\textcolor{red}{REF}}}&{\leq} C_2 h^{3/2}\norm{u}_{H^2}  + C_4 h^{3/2} 
	\end{align*}
	\textcolor{red}{QUESTION: How do I proceed from here?}
\end{proof}
\end{theorem}
...
\begin{theorem}$H^1$ Error Estimate \newline
	...
	\begin{proof}
		...
		\overset{\text{\textcolor{red}{REF}}}&{\leq} \norm{u-R_h(u)}_{H^1(\Omega)} + C_1 h^{3/2} \\
		\overset{\eqref{eq:ritz_minimization}}&{\leq} \norm{I_h(u) - R_h(u)}_{H^1(\Omega)} + C_1 h^{3/2} \\
		\overset{\text{\textcolor{red}{REF}}}&{\leq} C_2 h + C_1 h^{3/2}
		\end{align*}
	\end{proof}
\end{theorem}
...
\begin{definition} The Dual Problem \newline
	Let $u \in H^2(\Omega)$ be a solution of \eqref{eq:variational_inequality} for some $f \in L^2(\Omega)$ and $u_h \in V_h$ the solution of \eqref{eq:discrete_variational_inequality} for the same $f$. Furthermore, let $L \coloneqq \left\{ v \in H^1(-1,1) \,|\, v(\pm 1) \geq 0 \,\,\text{if}\,\, u(\pm 1) = 0 \right\}$.Then we define the dual problem as follows:
...
\begin{lemma}Existence of a Unique Dual Solution \newline 
	Let $u \in H^2(\Omega)$ be a solution of \eqref{eq:variational_inequality} for some $f \in L^2(\Omega)$ and $u_h \in V_h$ the solution of \eqref{eq:discrete_variational_inequality} for the same $f$. Then the dual problem \eqref{eq:dual_variational_inequality} admits a unique solution $z \in H^2(\Omega)$ for all $h > 0$. Furthermore, for this solution it holds that:
